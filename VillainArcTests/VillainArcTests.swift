...
struct VillainArcTests {
    ...
    @MainActor @discardableResult private func insertSuggestionEvent(
        ...
        let event = SuggestionEvent(
            category: category, catalogID: exercise.catalogID, sessionFrom: nil, targetExercisePrescription: exercise, targetSetPrescription: targetSet, triggerTargetSetID: targetSet?.id, decision: decision, outcome: outcome, trainingStyle: .straightSets, evaluatedAt: evaluatedAt, changes: changes)
    ...
    func userEditsDeleteMatchingUnresolvedSuggestionEvent() throws {
        ...
        let weightRuleChangeID = data.changes.first { $0.changeType == .increaseWeight && $0.event?.targetSetPrescription?.id == data.benchSet1.id }?.id
        let repsRuleChangeID = data.changes.first { $0.changeType == .decreaseReps && $0.event?.targetSetPrescription?.id == data.benchSet1.id }?.id
    ...
    func deletingSetDeletesPendingChangesForRemovedSet() throws {
        ...
        let ruleChangeForSet2ID = data.changes.first { $0.changeType == .increaseWeight && $0.previousValue == 155 && $0.event?.targetExercisePrescription?.id == data.bench.id }?.id
    ...
    func repRangeEditDoesNotOverridePendingSetChangeOutcome() throws {
        ...
        if let repRange = copyIncline.repRange { repRange.targetReps = repRange.targetReps + 2 }
        finishEditing(editCopy, originalPlan: data.plan, context: context)
    ...
    func replacingExerciseDeletesPendingOutcomeChangesForOriginalExercise() throws {
        ...
        let originalPendingChangeIDs = Set(data.changes.filter { $0.event?.targetExercisePrescription?.id == data.incline.id && $0.event?.outcome == .pending }.map(\.id))
    ...
    func readdingDeletedTailSetRestoresOriginalSetIdentity() throws {
        ...
        let tailSetChangeID = data.changes.first { $0.event?.targetSetPrescription?.id == data.benchSet2.id }?.id
        #expect(tailSetChangeID != nil)

        let editCopy = data.plan.createEditingCopy(context: context)
        guard let copyBench = (editCopy.exercises ?? []).first(where: { $0.id == data.bench.id }), let copySet2 = (copyBench.sets ?? []).first(where: { $0.id == data.benchSet2.id }) else {
    ...
    func editingRestoredTailSetDeletesMatchingUnresolvedChange() throws {
        ...
        let tailSetChangeID = data.changes.first { $0.event?.targetSetPrescription?.id == data.benchSet2.id && $0.changeType == .increaseWeight }?.id
        #expect(tailSetChangeID != nil)
