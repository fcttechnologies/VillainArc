...
@Suite(.serialized) struct OutcomeResolverGroupingTests {
    ...
    @MainActor private func makeBundle(context: ModelContext, prescription: ExercisePrescription, weightOld: Double, weightNew: Double, repsOld: Int, repsNew: Int, requiredEvaluationCount: Int = 1) -> SuggestionEvent {
        ...
        let triggerPerf = TestDataFactory.makePerformance(context: context, session: triggerSession, prescription: prescription, sets: [(weight: weightOld, reps: repsOld, rest: 90, type: .working)])
        ...
        let event = SuggestionEvent(
            ...
            requiredEvaluationCount: requiredEvaluationCount, createdAt: Date().addingTimeInterval(-3600), changes: [weightChange, repsChange])
    ...
    @Test @MainActor func bundle_primaryWeightIgnored_secondaryRepsGood_resolvesAsIgnored() async throws {
        ...
        let event = makeBundle(context: context, prescription: prescription, weightOld: 100, weightNew: 110, repsOld: 10, repsNew: 8)
        ...
        #expect(event.outcome == .ignored, "Primary weight change was not followed — secondary reps-good must not override")
    }
    ...
    @Test @MainActor func bundle_primaryGood_secondaryRepsIgnored_resolvesAsGood() async throws {
        ...
        let event = makeBundle(context: context, prescription: prescription, weightOld: 100, weightNew: 102.5, repsOld: 10, repsNew: 8)

        let session = TestDataFactory.makeSession(context: context)
        session.workoutPlan = plan
        // Athlete: followed weight (102.5), but kept old reps (10 = in-range but not near new target 8).
        _ = TestDataFactory.makePerformance(context: context, session: session, prescription: prescription, sets: [(weight: 102.5, reps: 10, rest: 90, type: .working)])

        await OutcomeResolver.resolveOutcomes(for: session, context: context)

        #expect(event.outcome == .good, "Primary weight followed — secondary reps-ignored must not demote the result")
    }
    ...
    @Test @MainActor func noPrimaryChanges_fallsBackToSeverityPriority_goodBeatsIgnored() async throws {
        ...
        let triggerSession = TestDataFactory.makeSession(context: context, daysAgo: 3)
        triggerSession.statusValue = .done
        let triggerPerf = TestDataFactory.makePerformance(context: context, session: triggerSession, prescription: prescription, sets: [(weight: 100, reps: 10, rest: 90, type: .working)])
        ...
        let event = SuggestionEvent(
            ...
            createdAt: Date().addingTimeInterval(-3600), changes: [repsChange, restChange])
        repsChange.event = event
        ...
        #expect(event.outcome == .good, "No primary changes — severity-priority fallback should pick good over ignored")
    }
