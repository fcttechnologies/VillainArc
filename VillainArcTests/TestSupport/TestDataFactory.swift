...
@MainActor enum TestDataFactory {
    ...
    static func makePerformance(context: ModelContext, session: WorkoutSession, prescription: ExercisePrescription, sets: [(weight: Double, reps: Int, rest: Int, type: ExerciseSetType)]) -> ExercisePerformance {
        let perf = ExercisePerformance(workoutSession: session, exercisePrescription: prescription)
        for set in perf.sets ?? [] { context.delete(set) }
        perf.sets?.removeAll()
...
@MainActor extension SetPerformance {
    ...
    fileprivate convenience init(rawIndex: Int) {
