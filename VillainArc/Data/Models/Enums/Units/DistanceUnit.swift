...
enum DistanceUnit: String, CaseIterable, Codable {
    ...
    nonisolated static var systemDefault: DistanceUnit {
    ...
    func display(_ meters: Double, fractionDigits: ClosedRange<Int> = 0...2) -> String {
        "\(fromMeters(meters).formatted(.number.precision(.fractionLength(fractionDigits)))) \(rawValue)"
    }
}
