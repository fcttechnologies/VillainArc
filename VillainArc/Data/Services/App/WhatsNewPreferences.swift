...
// Key: whats_new_last_shown_version — String, the last bundle short version the Welcome / What's New sheet was shown for.
...
nonisolated enum WhatsNewPreferences {
    ...
    // is stored we still treat them as returning (What's New) rather than brand-new (Welcome).
    ...
    // - brand-new install (no stored version, no legacy marker) → Welcome
    ...
    static func presentationOnLaunch() -> WhatsNewPresentation? {
        ...
        if stored == nil && !isReturningPreV14User {
            return WhatsNewPresentation(kind: .welcome, features: WhatsNewCatalog.welcomeHighlights)
        }

        let features = WhatsNewCatalog.featuresIntroduced(after: stored ?? "0", throughIncluding: current)
        guard !features.isEmpty else { return nil }
        return WhatsNewPresentation(kind: .whatsNew(version: current), features: features)
    }
