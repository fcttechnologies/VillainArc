...
struct HealthWorkoutDetailSummary: Equatable {
    ...
    var totalDuration: TimeInterval {
        max(endDate.timeIntervalSince(startDate), activeDuration)
    }
    
    var pausedDuration: TimeInterval {
        max(0, totalDuration - activeDuration)
    }
...
final class HealthWorkoutDetailLoader {
    ...
    private func loadEffort(for workout: HKWorkout) async -> HealthWorkoutEffortSummary? {
        do {
            ...
            let relatedSamples = result.relationships
                .filter { $0.workout.uuid == workout.uuid }
                .flatMap { $0.samples ?? [] }
            if let summary = makeEffortSummary(from: relatedSamples) {
                return summary
            }
        } catch {
            print("Failed to load workout effort relationship for \(cachedWorkout.healthWorkoutUUID): \(error)")
        }
    ...
    private func loadHeartRate(for workout: HKWorkout) async throws -> (HealthWorkoutHeartRateSummary, [HealthWorkoutHeartRatePoint], [HealthWorkoutHeartRateSample]) {
        ...
        let heartRateSamples = samples
            .map { sample in
                HealthWorkoutHeartRateSample(startDate: sample.startDate, endDate: sample.endDate, bpm: sample.quantity.doubleValue(for: bpmUnit))
            }
            .filter { $0.bpm > 0 }
            .sorted { $0.representativeDate < $1.representativeDate }
        
        let points = heartRateSamples
            .map { sample in
                HealthWorkoutHeartRatePoint(date: sample.representativeDate, bpm: sample.bpm)
            }
    ...
    private func loadRoute(for workout: HKWorkout) async throws -> [HealthWorkoutRoutePoint] {
        ...
        for route in routes {
            ...
            for try await location in routeDescriptor.results(for: healthStore) {
                ...
                if let last = coordinates.last, abs(last.latitude - coordinate.latitude) < 0.000_001, abs(last.longitude - coordinate.longitude) < 0.000_001 {
                    continue
                }
        ...
        return downsampledRouteCoordinates(from: coordinates, maxPoints: 600).map {
            HealthWorkoutRoutePoint(latitude: $0.latitude, longitude: $0.longitude)
        }
    }
    ...
    private func loadMetrics(for workout: HKWorkout) -> [HealthWorkoutDetailMetric] {
        ...
        if let flights = workout.statistics(for: flightsClimbedType)?
            .sumQuantity()?
            .doubleValue(for: .count()), flights > 0 {
        ...
        if let strokes = workout.statistics(for: swimmingStrokeCountType)?
            .sumQuantity()?
            .doubleValue(for: .count()), strokes > 0 {
    ...
    private func appendAverageMetric(title: String, type: HKQuantityTypeIdentifier, unit: HKUnit, style: HealthWorkoutDetailMetric.ValueStyle, from workout: HKWorkout, to items: inout [HealthWorkoutDetailMetric]) {
        ...
        guard let value = workout.statistics(for: quantityType)?
            .averageQuantity()?
            .doubleValue(for: unit), value > 0 else {
            return
        }
    ...
    private func heartRateBounds(for zone: Int, estimatedMaxHeartRate: Double) -> (Double?, Double?) {
        switch zone {
        case 1:
            return (nil, estimatedMaxHeartRate * 0.6)
        case 2:
            return (estimatedMaxHeartRate * 0.6, estimatedMaxHeartRate * 0.7)
        case 3:
            return (estimatedMaxHeartRate * 0.7, estimatedMaxHeartRate * 0.8)
        case 4:
            return (estimatedMaxHeartRate * 0.8, estimatedMaxHeartRate * 0.9)
        case 5:
            return (estimatedMaxHeartRate * 0.9, nil)
        default:
            return (nil, nil)
        }
    }
    ...
    private func midpoint(between start: Date, and end: Date) -> Date {
    ...
    private func makeEffortSummary(from samples: [HKSample]) -> HealthWorkoutEffortSummary? {
        ...
        if let workoutEffortType = HKQuantityType.quantityType(forIdentifier: .workoutEffortScore),
           let actualSample = quantitySamples.first(where: { $0.quantityType == workoutEffortType }) {
            let value = actualSample.quantity.doubleValue(for: .appleEffortScore())
            if value > 0 {
                return HealthWorkoutEffortSummary(source: .actualScore, value: value)
            }
        }
        
        if let estimatedEffortType = HKQuantityType.quantityType(forIdentifier: .estimatedWorkoutEffortScore),
           let estimatedSample = quantitySamples.first(where: { $0.quantityType == estimatedEffortType }) {
            let value = estimatedSample.quantity.doubleValue(for: .appleEffortScore())
            if value > 0 {
                return HealthWorkoutEffortSummary(source: .estimatedScore, value: value)
            }
        }
    ...
    private func fallbackEffortSummary(from workout: HKWorkout) -> HealthWorkoutEffortSummary? {
        if let workoutEffortType = HKQuantityType.quantityType(forIdentifier: .workoutEffortScore),
           let workoutEffort = workout.statistics(for: workoutEffortType)?
            .averageQuantity()?
            .doubleValue(for: .appleEffortScore()), workoutEffort > 0 {
            return HealthWorkoutEffortSummary(source: .actualScore, value: workoutEffort)
        }
        
        if let estimatedEffortType = HKQuantityType.quantityType(forIdentifier: .estimatedWorkoutEffortScore),
           let estimatedEffort = workout.statistics(for: estimatedEffortType)?
            .averageQuantity()?
            .doubleValue(for: .appleEffortScore()), estimatedEffort > 0 {
            return HealthWorkoutEffortSummary(source: .estimatedScore, value: estimatedEffort)
        }
        
        if let physicalEffortType = HKQuantityType.quantityType(forIdentifier: .physicalEffort),
           let physicalEffort = workout.statistics(for: physicalEffortType)?
            .averageQuantity()?
            .doubleValue(for: HKUnit(from: "kcal/(kg*hr)")), physicalEffort > 0 {
    ...
    private func markCachedWorkoutUnavailable() {
        ...
        if let context = cachedWorkout.modelContext {
