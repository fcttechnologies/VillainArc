...
enum SetupGuard {
    static func requireReady(context: ModelContext) throws {
        ...
        guard (try context.fetch(AppSettings.single).first) != nil else {
            throw SetupGuardError.onboardingNotComplete
        }

        guard let profile = try context.fetch(UserProfile.single).first else {
            throw SetupGuardError.onboardingNotComplete
        }
    ...
    static func requireNoActiveFlow(context: ModelContext) throws {
        ...
        if (try? context.fetch(WorkoutSession.incomplete).first) != nil {
