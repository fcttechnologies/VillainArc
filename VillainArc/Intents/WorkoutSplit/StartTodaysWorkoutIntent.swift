...
struct StartTodaysWorkoutIntent: AppIntent {
    ...
    @MainActor func perform() async throws -> some IntentResult & OpensIntent {
        ...
        guard let todaysDay = split.todaysSplitDay else { throw StartTodaysWorkoutError.noDayForToday }
        // Check if rest day
        guard !todaysDay.isRestDay else { throw StartTodaysWorkoutError.todayIsRestDay }
        // Check workout plan exists for today
        guard let workoutPlan = todaysDay.workoutPlan else { throw StartTodaysWorkoutError.noWorkoutPlanForToday }
