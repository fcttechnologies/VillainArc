...
struct ShowExerciseHistoryIntent: AppIntent {
    ...
    static var parameterSummary: some ParameterSummary {
        Summary("Show history for \(\.$exercise)")
    }

    @Parameter(title: "Exercise", requestValueDialog: IntentDialog("Which exercise history would you like to open?"))
    ...
    func perform() async throws -> some IntentResult & OpensIntent {
        ...
        guard let storedExercise = try context.fetch(Exercise.withCatalogID(catalogID)).first else {
            throw ShowExerciseHistoryError.exerciseNotFound
        }
        guard (try? context.fetch(ExerciseHistory.forCatalogID(catalogID)).first) != nil else {
            throw ShowExerciseHistoryError.noExerciseHistoryFound
        }
...
enum ShowExerciseHistoryError: Error, CustomLocalizedStringResourceConvertible {
    ...
    var localizedStringResource: LocalizedStringResource {
        ...
        case .noExerciseHistoryFound:
            return "Complete this exercise in a workout before opening its history."
        }
    }
}
