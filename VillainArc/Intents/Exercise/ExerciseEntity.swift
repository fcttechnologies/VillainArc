...
struct ExerciseEntityQuery: EntityQuery, EntityStringQuery {
    @MainActor func entities(for identifiers: [ExerciseEntity.ID]) async throws -> [ExerciseEntity] {
    ...
    @MainActor func suggestedEntities() async throws -> [ExerciseEntity] {
        ...
        if recentExercises.count >= 30 { return Array(recentExercises.prefix(30)).map(ExerciseEntity.init) }

        let remainingCount = max(0, 30 - recentExercises.count)
        let fallbackExercises = remainingCount == 0 ? [] : ((try? context.fetch(Exercise.backfillExcludingCatalogIDs(recentCatalogIDs, limit: remainingCount))) ?? [])
    ...
    @MainActor func entities(matching string: String) async throws -> [ExerciseEntity] {
        ...
        let results = searchedExercises(in: exercises, query: string, orderedBy: ordering.isOrderedBefore, score: { exercise, query, queryTokens in exerciseEntitySearchScore(for: exercise, query: query, queryTokens: queryTokens) }, fuzzyAdditionalTokens: { exercise in exerciseEntitySearchTokens(for: exercise) })
...
func exerciseEntitySearchScore(for exercise: Exercise, query: String, queryTokens: [String]? = nil) -> Int {
    ...
    if normalizedQuery.isEmpty { return score }
    ...
    for candidate in exactCandidates {
        ...
        if normalizedCandidate.hasPrefix(normalizedQuery + " ") { score += 500 }
    }
