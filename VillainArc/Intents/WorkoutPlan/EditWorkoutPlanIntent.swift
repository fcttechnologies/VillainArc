...
struct EditWorkoutPlanIntent: AppIntent {
    ...
    static var parameterSummary: some ParameterSummary {
        Summary("Edit \(\.$workoutPlan)")
    }

    @Parameter(title: "Workout Plan", requestValueDialog: IntentDialog("Which workout plan would you like to edit?"))
    ...
    func perform() async throws -> some IntentResult & OpensIntent {
        ...
        guard let storedPlan = try context.fetch(descriptor).first else {
            throw EditWorkoutPlanError.workoutPlanNotFound
        }
        guard storedPlan.completed && !storedPlan.isEditing else {
            throw EditWorkoutPlanError.workoutPlanIncomplete
        }
...
enum EditWorkoutPlanError: Error, CustomLocalizedStringResourceConvertible {
    ...
    var localizedStringResource: LocalizedStringResource {
        ...
        case .workoutPlanIncomplete:
            return "Finish creating the workout plan before editing it."
        }
    }
}
